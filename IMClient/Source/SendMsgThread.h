...
// 发送消息的线程，只是将数据包发送到真正发送数据的消息缓冲区里
...
private:
	...
	std::vector<CMsgItem*>		m_arrItem; // 没有使用

	std::list<CNetData*>		m_listItems;
    std::mutex                  m_mtItems;
    std::condition_variable     m_cvItems;	// 条件变量，用于阻塞或通知线程 https://www.cnblogs.com/fenghualong/p/13855360.html
