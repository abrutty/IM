	...
	for (int i = 0; i < nThreadNums; i++)
	{
		...
		if (lpThreadInfo != NULL)
		{
			hThread = (HANDLE)(ULONG_PTR)_beginthreadex(NULL, 0,  
				&ThreadProc, lpThreadInfo, CREATE_SUSPENDED, (UINT*)&dwThreadID); // CREATE_SUSPENDED 作用是线程创建并分配了资源，但没有运行
			if (hThread != NULL)
			{
				...
				::ResumeThread(hThread); // 执行这个函数，线程才会运行
			}
			else
			{
				delete lpThreadInfo;
	...
	for (int i = 0; i < (int)m_arrThreadInfo.size(); i++)
	{
		...
		if (lpThreadInfo != NULL)
		{
			...
			if (STILL_ACTIVE == dwExitCode)
				::TerminateThread(lpThreadInfo->hThread, 0); // 强制结束线程
			delete lpThreadInfo;
		}
	}
...
UINT CThreadPool::ThreadProc(LPVOID lpParam)
{
	...
	hWaitEvent[1] = lpThis->m_hSemaphore_Task; // 触发WAIT_OBJECT_1，增加、删除任务时才会

	for (;;)
	{
		dwIndex = ::WaitForMultipleObjects(2, hWaitEvent, FALSE, INFINITE); // FALSE 表明任何一个发出信号，就结束

		if (dwIndex == WAIT_OBJECT_0)	// 接到退出线程通知事件，m_hEvent_Exit触发的
			break;
	...
	if (lpThis->m_lThreadNums <= 0)		// 所有线程都结束完毕，置事件句柄有信号
		::SetEvent(lpThis->m_hEvent_ExitAll); // 和析构UnInit 中的事件关联
