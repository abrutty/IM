...
int main(int argc, char* argv[])
{
    ...
    //初始化数据库配置
    const char* dbserver = "192.168.152.131";
    const char* dbuser = "root";
    const char* dbpassword = "root";
    const char* dbname = "myim";
    //CMysqlManager 只关注用户名和密码，以及数据库是否存在？ 所有的真真正正的数据库操作交给成员m_poConn
    ...
    // 数据库初始化、用户初始化、服务器初始化后，就开启muduo库的无尽循环
    g_mainLoop.loop();  // loop 本身就是个线程，是一个反应堆，会去循环关注有无事件发生，有就处理，没有就等待
