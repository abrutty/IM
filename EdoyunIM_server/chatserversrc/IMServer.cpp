...
bool IMServer::Init(const char* ip, short port, EventLoop* loop)
{
    ...
    if (!Singleton<UserManager>::Instance().Init("192.168.152.131", "root", "root", "myim"))
    {
    ...
    m_server->setConnectionCallback(std::bind(&IMServer::OnConnection, this, std::placeholders::_1)); // 有连接来就调用OnConnection
    //启动侦听
    m_server->start();
...
{
    LOG_INFO << "OnConnection begin";
    if (conn->connected())
    {
        LOG_INFO << "client connected:" << conn->peerAddress().toIpPort();
        ++ m_baseUserId;
        std::shared_ptr<ClientSession> spSession(new ClientSession(conn)); // 客户端连上来就创建一个会话
        LOG_INFO << "connected client id is ：" << spSession->GetUserId();
        // 收到数据的回调函数，OnRead
